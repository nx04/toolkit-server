...
server_start_event(){
    docker run -p 58080:8080 --restart=always --net=docker_network --ip=172.19.80.80 --name gitbit_server -v /data/gitbit_server/data:/var/opt/gitblit -e TZ=Asia/Shanghai -d gitblit/gitblit:rpc --httpsPort=0
}
server_reload_event(){
...
# 拉取镜像
if [[ "$(docker images -q gitblit/gitblit:rpc 2> /dev/null)" == "" ]];then
